#include "Hyper.h"
#include "TMA.h"

std::vector<std::vector<double>> Hyper::Process(double c, double s, func2 F,
    func u0, func u1, func u2, func v0,
    size_t Nt, size_t Nl)
{
    std::vector<std::vector<double>> u(Nt+1, std::vector<double>(Nl+1)); // first time, next x

    double h = 1. / Nl;
    double dt = 2. / Nt;

    double q = h*h / dt / dt;

    for (size_t n = 0; n <= Nl; n++)
        u[0][n] = u0(n*h);

    u[1][0] = u1(dt);
    u[1][Nl] = u2(dt);
    for (size_t n = 1; n < Nl; n++)
        u[1][n] = u[0][n] + dt*v0(h*n) + dt*dt / 2 * (c*c*(u[0][n+1]-2*u[0][n]+u[0][n-1]) / h / h + F(0, h*n));

    for (size_t k = 1; k < Nt; k++) // time loop
    {
        TMA tma(Nl - 1);
        u[k+1][0] = u1((k+1)*dt);
        u[k+1][Nl] = u2((k+1)*dt);

        for (size_t n = 1; n < Nl; n++)
        {
            if (n == 1)
                tma.Iterate(0, 1/dt/dt + 2*s/h/h, -s/h/h,
                2 * u[k][n] / dt / dt - u[k - 1][n] / dt / dt + (1 - 2 * s)*(u[k][n + 1] - 2 * u[k][n] + u[k][n - 1]) / h / h + s*(u[k - 1][n + 1] - 2 * u[k - 1][n] + u[k - 1][n - 1]) / h / h
                + (s*F((k + 1)*dt, n*h) + (1 - 2 * s)*F(k*dt, n*h) + s*F((k - 1)*dt, n*h))+s/h/h*u[k+1][n-1]);
            else if (n == Nl - 1)
                tma.Iterate(-s/h/h, 1 / dt / dt + 2 * s / h / h, 0,
                2 * u[k][n] / dt / dt - u[k - 1][n] / dt / dt + (1 - 2 * s)*(u[k][n + 1] - 2 * u[k][n] + u[k][n - 1]) / h / h + s*(u[k - 1][n + 1] - 2 * u[k - 1][n] + u[k - 1][n - 1]) / h / h
                + (s*F((k + 1)*dt, n*h) + (1 - 2 * s)*F(k*dt, n*h) + s*F((k - 1)*dt, n*h)) + s / h / h*u[k + 1][n + 1]);
            else
                tma.Iterate(-s/h/h, 1/dt/dt + 2*s/h/h, -s/h/h,
                2*u[k][n]/dt/dt - u[k - 1][n]/dt/dt + (1 - 2*s)*(u[k][n + 1] - 2 * u[k][n] + u[k][n - 1])/h/h + s*(u[k - 1][n + 1] - 2 * u[k - 1][n] + u[k - 1][n - 1])/h/h
                + (s*F((k + 1)*dt, n*h) + (1 - 2 * s)*F(k*dt, n*h) + s*F((k - 1)*dt, n*h)));
        }
        std::vector<double> v = tma.Result();
        for (size_t n = 1; n < Nl; n++)
            u[k + 1][n] = v[n - 1];
    }

    return u;
}