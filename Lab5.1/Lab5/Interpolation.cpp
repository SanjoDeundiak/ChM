...
Spline Interpolation::Spline3() const
{
	const int c = 4;
	size_t n = X_.size(), k = c * (n - 1);
	vector<vector<Item>> A(k, vector<Item>(k, 0));
	...
	for (size_t i = 1; i < n - 1; i++) // 2nd derivatives
	{
		for (size_t j = 2; j < c; j++)
		{
			double a = pow(X_[i], j - 2), b = j * (j - 1) * a;
			A[i + 3 * n - 5][j + c*(i - 1)] = b;
			A[i + 3 * n - 5][j + c * i] = -b;
		}
		b[i + 3 * n - 5] = 0;
	...
	for (size_t j = 2; j < c; j++) // edge condition
	{
		A[4 * n - 6][j] = j * (j - 1) * pow(X_[0], j - 2);
		A[4 * n - 5][j] = j * (j - 1) * pow(X_[n - 1], j - 2);
	}
	b[4 * n - 6] = 2;
	b[4 * n - 5] = 0.357282;
